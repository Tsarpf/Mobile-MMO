...
module.exports = function(schema, options) {
    ...
    options.emailExistsError = options.emailExistsError || 'Email %s is already used';
    ...
    schema.statics.setEmail = function() {
        var self = this;

        return function(username, email, cb) {
            self.findByUsername(username, function(err, user) {
                if(err) {return cb(err); }

                if(user) {
                    return user.setEmail(email, cb);
                }
                else {
    ...
    schema.methods.setEmail = function(email, cb) {
        if(!email) {
            console.log("ääh missä email");
            return;
        }

        var self = this;

        self.set(options.emailField, email);
        cb(null, self);
    };
    ...
    schema.statics.authenticate = function() {
        ...
        return function(username, password, cb) {
            console.log("authenticating at fuck you");
            console.log("username: " + username);
            self.findByUsername(username, function(err, user) {
                console.log("find by username callback");
                if (err) { return cb(err); }
    ...
    schema.statics.register = function(user, password, email, cb) {
        ...
        if (!user.get(options.emailField)) {
            return cb(new BadRequestError(util.format(options.missingEmailError, options.emailField)));
        }

        var self = this;
        self.findByEmail(user.get(options.emailField), function(err, existingEmail) {
            if(err) { return cb(err);}
